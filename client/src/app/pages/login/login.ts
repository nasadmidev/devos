...
devos@system:~$ █`;
  ...
  protected loginForm = form(this.loginSchema, (schemaPath) => {
    ...
    submission: {
      action: async () => {
        ...
        try {
          const token = await lastValueFrom(this.http.post<ApiResponse<TokenResponse>>(`${environment.apiUrl}/auth/login`, { identity: email, password }))
          console.log(token)
